...
const FinancingRequest = {
    ...
    async loadExistingRequest() {
        try {
            const result = await API.financing.getRequest(this.requestId);
            if (result.success) {
    ...
    async submitRequest() {
        ...
        try {
            ...
            if (this.requestId) {
                ...
                result = await API.financing.updateRequest(this.requestId, requestData);
            } else {
                // Crear nueva solicitud
                result = await API.financing.createRequest(requestData);
            }
    ...
    async submitForReview(requestId) {
        const result = await API.financing.submitRequest(requestId);
