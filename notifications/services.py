...
def send_password_reset_email(user: User, reset_token: str) -> bool:
    ...
    reset_url = f"https://llevateloexpress.com/reset-password.html?token={reset_token}"
    
    context = {
        'reset_url': reset_url,
        'reset_token': reset_token,
        'expires_hours': 24,
        'site_name': 'LlévateloExpress'
    }
    return notification_service.send_notification(user, 'password_reset', context)
...
def send_password_changed_email(user: User) -> bool:
    ...
    return notification_service.send_notification(user, 'password_changed', context) 