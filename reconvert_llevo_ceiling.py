#!/usr/bin/env python
"""
Script para reconvertir precios LLEVO usando redondeo ceiling (hacia arriba)
"""
...
def reconvert_with_ceiling():
    """Reconvertir todos los precios USD a LLEVO usando ceiling"""
    print("🔄 Reconvirtiendo precios USD → LLEVO con redondeo ceiling...")
    ...
    for product in products:
        try:
            if product.price:
                # Calcular precio LLEVO con ceiling
                llevo_decimal = product.price * conversion_factor
                old_llevo = product.price_llevo or 0
                new_llevo = math.ceil(llevo_decimal)
                ...
                print(f"   Decimal: {llevo_decimal:.2f}")
    ...
    print("=" * 80)
    print(f"🎉 Reconversión ceiling completada: {converted}/{total_products}")
    
    # Mostrar algunos ejemplos de verificación
    print("\n🔍 VERIFICACIÓN - Ejemplos de ceiling:")
    examples = [
        (500, "Ejemplo $500"),
        (1000, "Ejemplo $1,000"),
        (2000, "Ejemplo $2,000"), 
    ...
    for usd_price, label in examples:
        llevo_decimal = usd_price * conversion_factor
        llevo_ceiling = math.ceil(llevo_decimal)
        print(f"{label}: {llevo_decimal:.2f} → {llevo_ceiling} LLEVO")

if __name__ == '__main__':
    try:
        reconvert_with_ceiling()
    except Exception as e:
